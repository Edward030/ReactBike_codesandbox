...
export default function StationPage () {
  ...
  const [curCheckbox, setCurCheckbox] = React.useState(checkbox_taipei);
  const [isCheck, setIsCheck] = React.useState(
    curCheckbox.map((li) => li.name)
  );
  ...
  const handleCityChange = (e) => {
    ...
    if (selectedCityValue === 'taipei') {
      setCurCheckbox(checkbox_taipei);
      ...
      setCurCheckbox(checkbox_other);
      setIsCheck(checkbox_other.map((li) => li.name));
      setData([]);
    }
  };
  const handleSubmit = () => {
  ...
  const handleCheck = (e) => {
    ...
    if (!checked) {
      ...
      setIsCheckAll(false);
    } else if (isCheck.length === curCheckbox.length - 1) {
      setIsCheckAll(true);
    }
  ...
  const handleSelectAll = (e) => {
    ...
    if (isCheckAll) {
      ...
      setIsCheck(curCheckbox.map((li) => li.name));
      fetchData().then((jsonData) => {
        setData(jsonData);
      });
    }
  ...
  React.useEffect(() => {
    fetchData().then((jsonData) => {
      const filteredData = jsonData.filter(
        (item) => isCheck.includes(item.sarea) && item.sna.includes(search)
      );
    ...
    <>
      ...
      <div id="fieldset">
        ...
        <input
          ...
          name="checkboxAll"
        />
        <div id="checkboxAllText">全部勾選</div>
        <CheckboxPanel
          checked={isCheck}
          onChange={handleCheck}
          checkboxArr={curCheckbox}
        />
        <TableComponent
          data={data}
...
const CheckboxPanel = ({checked, onChange, checkboxArr}) => {
  return (
    <div id="mid-panel">
      <div id="checkbox-panel">
        {checkboxArr.map(({ dist, name }) => (
          <>
            <input
              type="checkbox"
              id={dist}
      ...
      <img
        id="stationPic"
        src="https://i.imgur.com/dFK7idR.png"
        alt="stationPic"
      />
    </div>
  );
